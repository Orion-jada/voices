  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {crosswords.map(cw => (
          <Link href={`/crosswords/${cw.id}`} key={cw.id} className="block p-6 bg-white rounded shadow hover:shadow-lg transition border border-gray-200 text-center">
            <h2 className="text-2xl font-bold text-gray-800 mb-2">{cw.title}</h2>
            <p className="text-gray-500">{new Date(cw.created_at).toLocaleDateString()}</p>

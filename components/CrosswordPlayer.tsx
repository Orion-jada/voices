  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 justify-center">
        <div className="space-y-4">
             <div 
                style={{ display: 'grid', gridTemplateColumns: `repeat(${gridStructure[0].length}, 2rem)` }} 
                className="gap-px bg-gray-400 border border-black w-fit shadow-lg"
             >
                {gridStructure.map((row, r) => row.map((cell, c) => {
                    ...
                    return (
                        <div 
                            key={`${r}-${c}`} 
                            className={clsx(
                                "w-8 h-8 relative flex items-center justify-center text-lg font-bold uppercase select-none cursor-pointer",
                                cell.isBlack ? 'bg-black' : 'bg-white',
                                isSelected ? 'bg-yellow-200' : (isHighlighted && !cell.isBlack ? 'bg-blue-50' : '')
                            )}
                            onClick={() => handleCellClick(r, c)}
                        >
                            {cell.number && <span className="absolute top-0 left-0 text-[8px] leading-none p-0.5 pointer-events-none">{cell.number}</span>}
                            {!cell.isBlack && userGrid[r][c]}
                        </div>
                    );
                }))}
            </div>

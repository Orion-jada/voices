        ...
        <div className="space-y-8 pb-20">
            ...
            <div className="bg-white p-4 rounded shadow">
                ...
                <div className="flex gap-4 mb-4">
                    <button onClick={() => setMode('grid')} className={`px-4 py-2 rounded ${mode === 'grid' ? 'bg-black text-white' : 'bg-blue-100 text-blue-900 hover:bg-blue-200'}`}>Edit Grid Layout (Black Squares)</button>
                    <button onClick={() => setMode('letters')} className={`px-4 py-2 rounded ${mode === 'letters' ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-900 hover:bg-blue-200'}`}>Edit Solution (Letters)</button>
                </div>

                <div className="flex flex-col lg:flex-row gap-8">
                    <div className="overflow-auto">
                        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${grid[0]?.length || 1}, 2rem)` }} className="gap-px bg-black border border-black w-fit">
                            {grid.map((row, r) => row.map((cell, c) => (
                                ...
                                >
                                    ...
                                    {!cell.isBlack && mode === 'grid' && cell.value && (
                                        <span className="text-blue-200">{cell.value}</span>
                                    )}
                                </div>
                            )))}
                        </div>
